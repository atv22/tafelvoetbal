...
def test_generate_prinsjesdag_seasons_split():
    ...
    assert any('P→15 mrt' in s for s in seasons_df['seizoen_naam'])
    assert any('16 mrt→P' in s for s in seasons_df['seizoen_naam'])

    # 15 maart moet in het P→15 mrt blok zitten
    matched_15 = seasons_df[(seasons_df['start_datum'] <= pd.Timestamp('2025-03-15 23:59:59')) & (seasons_df['eind_datum'] >= pd.Timestamp('2025-03-15 23:59:59'))]
    assert not matched_15.empty
    assert 'P→15 mrt' in matched_15.iloc[0]['seizoen_naam']

    # 16 maart moet in het 16 mrt→P blok zitten
    matched_16 = seasons_df[(seasons_df['start_datum'] <= pd.Timestamp('2025-03-16 00:00:00')) & (seasons_df['eind_datum'] >= pd.Timestamp('2025-03-16 00:00:00'))]
    assert not matched_16.empty
    assert '16 mrt→P' in matched_16.iloc[0]['seizoen_naam']
